...
/* set up mode building blocks for selection list of days */
function setupDays(){
...
function updateDays(){
	...
	if (deliveryYear.selectedIndex === -1){
		// if no year is selceted , choose the default year so length of Feb can be determined
...
function copyBillingAddress(){
	...
	var deliveryInputElements = document.querySelectionAll("#deliveryAddress input");
	if(document.getElementById("sameAddr").checked){
		for (var i = 0; i < billingInputElements.length; i++){
			deliveryInputElements[i + 1].value = billingInputElements[i].value;
 		}
		document.querySelctor("#deliveryAddress select").value =
			document.querySelctor("#billingAddress select").value;
	} else {
		for (var i=0; i < billingInputElements.length; i++){
			deliveryInputElements[i + 1].value = "";
		}
		document.querySelctor("#deliveryAddress select").selectedIndex = -1;
	}
}
...
function validateAddress(fieldsetId) {
	var inputElements = document.querySelectionAll("#" + fieldsetId + "input");
	var errorDiv = document.querySelectionAll("#" + fieldsetId + " .errorMessage") [0];
	...
	var currentElement;
		...
		try {
			...
			currentElement = document.querySelctor("#" + fieldsetId + "select");
...
function validatePayment() {
	var errorDiv = document.querySelctor("#paymentInfo .errorMessage");
	var fieldsetValidity = true;
	var ccNumElement = document.getElementById("ccNum");
	var selectElements = document.querySelectionAll("#paymentInfo select");
	var elementCount = selectElements.length;
	var cvvElement = document.getElementById("cvv");
	var cards = document.getElementsByName("Payment Type");
	var currentElement;
...
function validateMessage() {
	var errorDiv = document.querySelctor("#message .errorMessage");
...
function validateCreateAccount() {
	var errorDiv = document.querySelctor("#createAccount .errorMessage");
	...
	try {
		...
		errorDiv.style.display = "none";
		if ((usernameElement.value !== "" && pass1Element !== "" && pass2Element.value !== "")) {
